...
public class VnfApiProvider implements AutoCloseable, VNFAPIService, DataChangeListener {
    ...
    public Future<RpcResult<VnfTopologyOperationOutput>> vnfTopologyOperation(VnfTopologyOperationInput input) {
        ...
        if (input == null || input.getServiceInformation() == null
            ...
            log.debug(EXITING_STR + SVC_OPERATION + " because of invalid input, null or empty service-instance-id");
            responseBuilder.setResponseCode("403");
        ...
        log.info(ADDING_INPUT_DATA_STR + SVC_OPERATION + " [" + siid + INPUT_STR + input);
        VnfTopologyOperationInputBuilder inputBuilder = new VnfTopologyOperationInputBuilder(input);
        VnfSdnUtil.toProperties(parms, inputBuilder.build());

        log.info(ADDING_OPERATIONAL_DATA_STR + SVC_OPERATION + " [" + siid + OPERATIONAL_DATA_STR + operDataBuilder
            .build());
        VnfSdnUtil.toProperties(parms, OPERATIONAL_DATA, operDataBuilder);
        ...
        try {
            if (svcLogicClient.hasGraph(VNF_API, SVC_OPERATION, null, "sync")) {
                ...
                try {
                    respProps =
                        svcLogicClient.execute(VNF_API, SVC_OPERATION, null, "sync", serviceDataBuilder, parms);
                } catch (Exception e) {
                    log.error("Caught exception executing service logic for " + SVC_OPERATION, e);
                    errorMessage = e.getMessage();
                    errorCode = "500";
        ...
        if (errorCode != null && errorCode.length() != 0 && !("0".equals(errorCode) || "200".equals(errorCode))) {
            ...
            try {
                ...
                log.error(CAUGHT_EXCEPTION_STR + SVC_OPERATION + " [" + siid + "] \n", e);
            }
            log.error(RETURNED_FAILED_STR + SVC_OPERATION + " [" + siid + "] " + responseBuilder.build());
            RpcResult<VnfTopologyOperationOutput> rpcResult =
                RpcResultBuilder.<VnfTopologyOperationOutput>status(true).withResult(responseBuilder.build()).build();
            // return error
            return Futures.immediateFuture(rpcResult);
        ...
        try {
            ...
            log.info(UPDATING_MD_SAL_STR + SVC_OPERATION + " [" + siid + "] ServiceData: " + serviceData);
            // svc-configuration-list
            VnfListBuilder vnfListBuilder = new VnfListBuilder();
        ...
        log.info(UPDATED_MD_SAL_STR + SVC_OPERATION + " [" + siid + "] ");
        log.info(RETURNED_SUCCESS_STR + SVC_OPERATION + " [" + siid + "] " + responseBuilder.build());
    ...
    public Future<RpcResult<NetworkTopologyOperationOutput>> networkTopologyOperation(
        ...
        try {
            if (svcLogicClient.hasGraph(VNF_API, SVC_OPERATION, null, "sync")) {
                ...
                try {
                    respProps =
                        svcLogicClient.execute(VNF_API, SVC_OPERATION, null, "sync", preloadDataBuilder, parms);
                } catch (Exception e) {
                    log.error("Caught exception executing service logic for " + SVC_OPERATION, e);
                    errorMessage = e.getMessage();
                    errorCode = "500";
    ...
    public Future<RpcResult<PreloadVnfTopologyOperationOutput>> preloadVnfTopologyOperation(
        ...
        if (input == null || input.getVnfTopologyInformation() == null
            ...
            log.debug(EXITING_STR + SVC_OPERATION + " because of invalid input, null or empty vnf-name or vnf-type");
            responseBuilder.setResponseCode("403");
        ...
        if (preload_name == null || preload_name.length() == 0) {
            log.debug(EXITING_STR + SVC_OPERATION + " because of invalid preload-name");
            responseBuilder.setResponseCode("403");
        ...
        if (preload_type == null || preload_type.length() == 0) {
            log.debug(EXITING_STR + SVC_OPERATION + " because of invalid preload-type");
            responseBuilder.setResponseCode("403");
        ...
        log.info(
            ADDING_INPUT_DATA_STR + SVC_OPERATION + " [" + preload_name + "," + preload_type + INPUT_STR + input);
        PreloadVnfTopologyOperationInputBuilder inputBuilder = new PreloadVnfTopologyOperationInputBuilder(input);
        VnfSdnUtil.toProperties(parms, inputBuilder.build());
        log.info(ADDING_OPERATIONAL_DATA_STR + SVC_OPERATION + " [" + preload_name + "," + preload_type
        ...
        if (errorCode != null && errorCode.length() != 0 && !("0".equals(errorCode) || "200".equals(errorCode))) {
            ...
            log.error(
                RETURNED_FAILED_STR + SVC_OPERATION + " [" + preload_name + "," + preload_type + "] error code: '"
                    + errorCode + "', Reason: '" + errorMessage + "'");
            try {
                savePreloadList(preloadVnfListBuilder.build(), true, LogicalDatastoreType.CONFIGURATION);
            } catch (Exception e) {
                log.error(
                    CAUGHT_EXCEPTION_STR + SVC_OPERATION + " [" + preload_name + "," + preload_type
                        + "] \n", e);
            }
        ...
        try {
            ...
            log.info(
                UPDATING_MD_SAL_STR + SVC_OPERATION + " [" + preload_name + "," + preload_type + "] preloadData: "
                    + preloadData);
            // svc-configuration-list
            VnfPreloadListBuilder preloadVnfListBuilder = new VnfPreloadListBuilder();
            preloadVnfListBuilder.setVnfName(preload_name);
            ...
            log.error(CAUGHT_EXCEPTION_STR + SVC_OPERATION + " [" + preload_name + "," + preload_type
                + "] \n", e);
            responseBuilder.setResponseCode("500");
            responseBuilder.setResponseMessage(e.toString());
            responseBuilder.setAckFinalIndicator("Y");
            log.error(RETURNED_FAILED_STR + SVC_OPERATION + " [" + preload_name + "," + preload_type + "] "
        ...
        log.info(UPDATED_MD_SAL_STR + SVC_OPERATION + " [" + preload_name + "," + preload_type + "] ");
        log.info(
            RETURNED_SUCCESS_STR + SVC_OPERATION + " [" + preload_name + "," + preload_type + "] " + responseBuilder
                .build());
    ...
    public Future<RpcResult<PreloadVnfInstanceTopologyOperationOutput>> preloadVnfInstanceTopologyOperation(
        ...
        if (input == null || input.getVnfInstanceTopologyInformation() == null
            ...
            log.debug(EXITING_STR + SVC_OPERATION
        ...
        try {
            if (svcLogicClient.hasGraph(VNF_API, SVC_OPERATION, null, "sync")) {
                ...
                try {
                    respProps = svcLogicClient
                        .execute(VNF_API, SVC_OPERATION, null, "sync", vnfInstancePreloadDataBuilder, parms);
                } catch (Exception e) {
                    log.error("Caught exception executing service logic for " + SVC_OPERATION, e);
                    errorMessage = e.getMessage();
                    errorCode = "500";
    ...
    public Future<RpcResult<PreloadVfModuleTopologyOperationOutput>> preloadVfModuleTopologyOperation(
        ...
        try {
            if (svcLogicClient.hasGraph(VNF_API, SVC_OPERATION, null, "sync")) {
                try {
                    respProps = svcLogicClient
                        .execute(VNF_API, SVC_OPERATION, null, "sync", vfModulePreloadDataBuilder, parms);
                } catch (Exception e) {
                    log.error("Caught exception executing service logic for " + SVC_OPERATION, e);
                    errorMessage = e.getMessage();
                    errorCode = "500";
    ...
    public Future<RpcResult<PreloadNetworkTopologyOperationOutput>> preloadNetworkTopologyOperation(
        ...
        if (input == null || input.getNetworkTopologyInformation() == null
            ...
            log.debug(EXITING_STR + SVC_OPERATION + " because of invalid input, null or" +
